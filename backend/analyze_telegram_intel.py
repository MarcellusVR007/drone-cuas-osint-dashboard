#!/usr/bin/env python3
"""
AI-Powered Telegram Intelligence Analysis
Translates, classifies, and scores Telegram posts for OSINT value
"""
...
3. **Intelligence Value** (0-10 scale):
   - 9-10: Direct evidence of surveillance requests, bounties for specific targets
...
5. **Confidence Level** (LOW/MEDIUM/HIGH/VERY_HIGH):
   - VERY_HIGH: Explicit surveillance requests, bounty offers with crypto addresses
   - HIGH: Clear recruitment, specific location mentions, payment details
   - MEDIUM: Indirect references, general discussion of operations
   - LOW: Speculative, unclear, or tangential mentions
...
def analyze_all_posts():
    """Analyze all Telegram posts in database"""

    print("=" * 80)
    print("AI-POWERED TELEGRAM INTELLIGENCE ANALYSIS")
    print("=" * 80)
    ...
    print(f"\n📊 Analyzing {len(posts)} Telegram posts with AI...\n")
    ...
    for post_id, channel, post_date, content, post_url in posts:
        ...
        print(f"   Translation: {analysis.get('translation', '')[:100]}...")
        ...
        if analysis.get('is_intelligence'):
            ...
            print(f"   ⚠️  Low intelligence value")
        ...
        cursor.execute("""
            ...
            WHERE id = ?
        """, (
            analysis.get('translation', content),  # Store English translation
        ...
        if analyzed % 10 == 0:
            conn.commit()
            print(f"💾 Checkpoint: {analyzed}/{len(posts)} posts analyzed\n")
    ...
    return analyzed, intelligence_posts
